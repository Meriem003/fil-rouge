...
function getfirstname(employees){
  if(employees.length > 0 && employees[0].firstName){
    return employees[0].firstName;
  }else{
    return null
  }
}
console.log(getfirstname(employees));

function getlastname(employees){
  i= employees.length-1
  if(employees.length> i && employees[i].lastName){
    return employees[i].lastName
  }else{
    return null
  }
}
console.log(getlastname(employees))
...
console.log(getEmployeeDepartmentById(employees))
...
function getHighestPaidEmployee(employees){
  let salary = 0
  let employeessalair = null
  for(let i=0; i < employees.length ; i++){
    if(employees[i].salary > salary){
      salary = employees[i].salary
      employeessalair = employees[i]
    }
  }
  return employeessalair
}
...
function getUniqueDepartments(employees){
  ...
  for (let i = 0; i < employees.length; i++) {
    ...
    for (let j = 0; j < count.length; j++) {
      if (department === count[j]) {
        exists = true;
        break;
      }
    }
    if (exists === false) {
      count[index] = department;
      index++;
    }
  }
  return count;
